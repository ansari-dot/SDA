...
export class HotelController {
    ...
    static async getHotel(req, res) {
        ...
        try {
            ...
            return res.status(200).json({ hotels }); // Changed to return object with hotels property to match frontend
        } catch (error) {
            console.error("Error fetching hotels:", error);
            return res.status(500).json({ message: "Server error", error: error.message });
        }
    ...
    static async getRoom(req, res) {
            try {
                ...
                const userId = req.user; // Handle both _id and id
                const hotelId = req.params.hotelId;
                ...
                // Verify the hotel belongs to the user
                ...
                console.error(`Error fetching rooms for hotel ${req.params.hotelId}:`, error);
